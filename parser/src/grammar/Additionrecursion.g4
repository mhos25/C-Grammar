...
assignment: variableName = IDENTIFIER '=' value=comparingBooleanAndArrayElement  #BooleanVariableEvaluationAndAssignment
            | varName=IDENTIFIER '=' expr=expression  #directVarAssignment
...
branch: 'if' '(' lft=expression operator = ('<'| '<=' | '>' |'>=') rght=expression ')' onTrue=block 'else' onFalse=block    #CompareStatements_if
        ...
        | 'if' '(' varName= IDENTIFIER ')' valid= statement 'else' nonValid= statement  #CheckforOne_If
